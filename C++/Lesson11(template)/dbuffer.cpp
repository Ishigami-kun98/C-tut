...
//lettura scritture
int &dbuffer :: operator[](int index){
  //Il contenuto è totalmente uguale alla value
  assert(index < size);
  return buffer[index];

}
//lettura
const int &dbuffer:: operator[](int index) const{
  assert(index < size);
  return buffer[index];
}
...
dbuffer::dbuffer(const dbuffer &other){
 ...
 std:: ostream & operator <<(std::ostream &os, const dbuffer &db){
   os << db.S() << std::endl;
   for (size_t i = 0; i < db.S(); i++) {
     os << db.buffer[i] << std:: endl;
   }
   return os;
 }
